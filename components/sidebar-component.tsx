...
function getSidebarContent(activeSection: string, pathname: string, viewParam: string | null): SidebarContent {
  const isActive = (path: string) => pathname === path || (path !== "/" && pathname.startsWith(path));
  const isAnalyticsView = viewParam === "analytics";

  const dashboard = (
    <Dashboard size={16} className="text-neutral-50" />
  );
      ...
      sections: [
        {
          title: "Main",
          items: [
            { icon: view, label: "Overview", href: "/dashboard", isActive: isActive("/dashboard") && !isAnalyticsView },
            { icon: docAdd, label: "Upload document", href: "/upload" },
            { icon: chatIcon, label: "Chat with Doc.", href: "/chat", isActive: pathname.startsWith("/chat") },
            { icon: analyticsIcon, label: "Analytics Dashboard", href: "/dashboard?view=analytics", isActive: isAnalyticsView },
          ],
        },
        {
          title: "Documents",
          items: [
            { icon: report, label: "All summaries", href: "/dashboard", isActive: isActive("/dashboard") && !isAnalyticsView },
          ],
        },
      ],
    },
      ...
      sections: [
        {
          title: "Documents",
          items: [
            { icon: cloudUpload, label: "Upload new document", href: "/upload", isActive: isActive("/upload") },
      ...
      sections: [
        {
          title: "Workspaces",
          items: [
            { icon: folderOpen, label: "My workspaces", href: "/workspaces", isActive: isActive("/workspaces") },
...
function IconNavigation({ activeSection }: { activeSection: string }) {
  ...
  return (
    <aside className="bg-black flex flex-col gap-2 items-center p-4 w-16 h-full border-r border-neutral-800 rounded-l-2xl shrink-0 overflow-hidden">
      <Link href="/" prefetch={true} scroll={false} className="mb-2 size-10 flex items-center justify-center rounded-full overflow-hidden border border-neutral-700 hover:bg-neutral-800 transition-colors shrink-0">
        {userImageUrl ? (
          <Image
            src={userImageUrl}
      ...
      <div className="flex flex-col gap-2 w-full items-center">
        {ICON_NAV_ITEMS.map((item) => (
          <Link
            ...
            prefetch={true}
            scroll={false}
            className={`flex items-center justify-center rounded-lg size-10 min-w-10 transition-colors duration-500 ${activeSection === item.id ? "bg-neutral-800 text-neutral-50" : "hover:bg-neutral-800 text-neutral-400 hover:text-neutral-300"
              }`}
...
function DetailSidebar({ activeSection, pathname, viewParam }: { activeSection: string; pathname: string; viewParam: string | null }) {
  ...
  return (
    <aside className="bg-black flex flex-col gap-4 items-start p-4 rounded-r-2xl h-full w-80 shrink-0 overflow-hidden">
      ...
      <div className="w-full mt-auto pt-2 border-t border-neutral-800 shrink-0 flex flex-col items-stretch relative z-10 overflow-visible pb-1">
        ...
        >
          <LogOut size={16} className="shrink-0 text-neutral-500 group-hover:text-neutral-50 transition-colors" aria-hidden />
          <span>Log out</span>
        </button>
      </div>
    </aside>
...
function MenuItem({
  ...
  const baseClass = `rounded-lg cursor-pointer transition-all duration-500 flex items-center relative w-full h-10 px-4 py-2 ${item.isActive ? "bg-neutral-800" : "hover:bg-neutral-800"
    }`;

  return (
    <div className="relative shrink-0 w-full">
      {item.href && !item.hasDropdown ? (
        <Link href={item.href} scroll={false} prefetch={true} className={baseClass} style={{ transitionTimingFunction: softSpringEasing }}>
          {content}
        </Link>
      ) : (
        <div className={baseClass} style={{ transitionTimingFunction: softSpringEasing }} onClick={handleClick}>
          {content}
        </div>
      )}
    </div>
...
function SubMenuItem({ item }: { item: MenuItemT }) {
  ...
  return (
    <div className="w-full pl-9 pr-1 py-[1px]">
      {item.href ? (
        <Link href={item.href} scroll={false} prefetch={true} className="h-10 w-full rounded-lg cursor-pointer transition-colors hover:bg-neutral-800 flex items-center px-3 py-1">
          {inner}
        </Link>
      ) : (
...
function getActiveSectionFromPathname(pathname: string): string {
  ...
  if (pathname.startsWith("/dashboard") || pathname.startsWith("/summaries") || pathname.startsWith("/chatbot") || pathname.startsWith("/chat")) return "dashboard";
  return "dashboard";
}
...
function TwoLevelSidebar({ className }: { className?: string }) {
  ...
  return (
    <div className={`sticky top-0 h-screen shrink-0 overflow-hidden ${className ?? "flex flex-row"}`}>

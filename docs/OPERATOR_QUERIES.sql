...
ORDER BY dv.created_at ASC,
  dc.chunk_index ASC;
-- Version Completion Status (all versions)
-- To filter by specific version, uncomment and modify the WHERE clause below
SELECT dv.id as version_id,
  dv.version_number,
  d.title as document_title,
  d.user_id,
  ...
  COUNT(dc.id) FILTER (
    WHERE dc.summary IS NOT NULL
  ) as chunks_with_summary,
  COUNT(dc.id) FILTER (
...
FROM document_versions dv
  JOIN documents d ON dv.document_id = d.id
  LEFT JOIN document_chunks dc ON dv.id = dc.document_version_id
-- Uncomment the line below and replace with actual version UUID to filter by specific version:
-- WHERE dv.id = 'a1b2c3d4-e5f6-7890-abcd-ef1234567890'
GROUP BY dv.id,
  dv.version_number,
  d.title,
  d.user_id,
...
WHERE dc.reused_from_chunk_id IS NOT NULL
  ...
  AND NOT EXISTS (
    SELECT 1
    FROM document_chunks source_dc
...
SELECT d.id as document_id,
  ...
  ROUND(
    AVG(
      dv.reused_chunks::float / NULLIF(dv.total_chunks, 0)
    ) * 100,
    2
  ) as avg_reuse_percent,
...
LIMIT 50;
SELECT DATE_TRUNC('day', dv.created_at) as date,
...
SELECT dc.id as chunk_id,
  ...
  dc.updated_at,
  CASE
    WHEN dc.summary IS NULL
    AND dc.reused_from_chunk_id IS NULL THEN 'incomplete'
    WHEN dc.summary IS NOT NULL THEN 'complete'
    WHEN dc.reused_from_chunk_id IS NOT NULL
    AND dc.summary IS NULL THEN 'reused_pending'
    ELSE 'unknown'
  END as status
FROM document_chunks dc
  JOIN document_versions dv ON dc.document_version_id = dv.id
...
SELECT dv.id as version_id,
  ...
  COUNT(dc.id) FILTER (
    WHERE dc.summary IS NULL
      AND dc.reused_from_chunk_id IS NULL
  ) as incomplete_new_chunks,
  COUNT(dc.id) FILTER (
    WHERE dc.summary IS NULL
      AND dc.reused_from_chunk_id IS NOT NULL
  ) as incomplete_reused_chunks,

...
import { AiInsightsPanel } from "@/components/summaries/ai-insights-panel";
import type { ContentInsightItem } from "@/app/api/documents/[id]/versions/[versionId]/insights/content/route";
import { Button } from "@/components/ui/button";
import { ChevronLeft } from "lucide-react";

export default async function SummaryAiInsightsPage(props: { params: Promise<{ id: string }> }) {
  ...
  const res = await fetch(
    `${base}/api/documents/${documentId}/versions/${versionId}/insights/content`,
    {
      cache: "no-store",
      ...
      <SummaryPageWrapper>
        <div className="min-h-[60vh] flex flex-col items-center justify-center px-4">
          <p className="text-[#888] text-center mb-4">
            {res.status === 404 ? "Version not found." : "Failed to load AI insights."}
          </p>
          <Button variant="outline" asChild className="border-[#333] text-[#888] hover:bg-[#1a1a1a]">
            <Link href={`/summaries/${summaryId}`}>
              <ChevronLeft className="w-4 h-4 mr-2" />
  ...
  const data: { insights: ContentInsightItem[] } = await res.json();
